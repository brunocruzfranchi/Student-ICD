...
df.frecuencia <- function(nombre_dataset, n_umbral, iteraciones, Método){
...
#_________________________ Analisis Ridge solo de Variables  #############################
...
# Plot
g10 <- ggplot(data = df.Math.Ridge, mapping = aes(x = Variables, y = Frecuencia, fill = Variables)) +
  geom_bar(stat = "identity")+
  geom_text(aes(label = Frecuencia), position = position_stack(vjust= 0.5),
            colour = "black", size = 3)+
  scale_fill_manual(values = getPalette(nrow(df.Math.Ridge)))+
  theme_light()+
  theme_minimal()

print(g10 + mynamestheme + labs( title= "Matemática - Ridge", y="Frecuencia", 
                                x = "Variables"))
...
# Plot
g12 <- ggplot(data = df.Por.Ridge, mapping = aes(x = Variables, y = Frecuencia, fill = Variables)) +
  geom_bar(stat = "identity")+
  geom_text(aes(label = Frecuencia), position = position_stack(vjust= 0.5),
            colour = "black", size = 3)+
  scale_fill_manual(values = getPalette(nrow(df.Por.Ridge)))+
  theme_light()+
  theme_minimal()

print(g12 + mynamestheme + labs( title= "Portugués - Ridge", y="Frecuencia", 
                                x = "Variables"))
...
#_________________________ Regresion Elastic Net ######################
...
#_________________________ Valores de Error CV para todas los Métodos ##############
